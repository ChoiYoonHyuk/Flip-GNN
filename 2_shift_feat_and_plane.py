...
class Net(torch.nn.Module):
    def __init__(self):
        ...
        self.t = torch.cat((torch.empty(dataset.num_node_features).fill_(-move).to(device), torch.zeros(1).to(device)))
    ...
    def forward(self, x, edge_index, idx):
        ...
        # If the boundary is non-flipped and the input is flipped (1 - data.x), adjust bias as below
            ...
            state[data.num_features] = refine
            model.state_dict()['conv1.weight'].data.copy_(state)
            self.k_value = 1
...
without_bias, with_bias = torch.cat((data.x, zero), 1), torch.cat((data.x + move, one), 1)
