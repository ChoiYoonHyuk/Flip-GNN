...
class Net(torch.nn.Module):
    def __init__(self):
        ...
        dim = 64
        self.conv1 = GINConv(Linear(dataset.num_features + 1, dim))
    ...
    def forward(self, x, edge_index, idx):
        err = 0
        state = model.state_dict()['conv1.nn.weight'].T.detach().clone()
        ...
        if idx == 0 and self.k_value == 1:
            ...
            model.state_dict()['conv1.nn.weight'].data.copy_(state.T)
...
for ee in tqdm(range(epoch)):
    ...
    for _ in range(300):    
        ...
        loss.backward()
        if idx == 0:
            model.conv1.nn.weight.grad = model.conv1.nn.weight.grad.clone() * alpha
        else:
            model.conv1.nn.weight.grad = -model.conv1.nn.weight.grad.clone() * beta
        optim.step()
